...
goto finish
:python_ready
%ESTIMATOR_PYTHON% -X utf8 -m venv .build-venv-windows
set "ESTIMATOR_EXIT=%ERRORLEVEL%"
if not "%ESTIMATOR_EXIT%"=="0" goto finish
".build-venv-windows\Scripts\python.exe" -X utf8 -m pip --isolated install --require-hashes -r requirements-lock.txt
set "ESTIMATOR_EXIT=%ERRORLEVEL%"
if not "%ESTIMATOR_EXIT%"=="0" goto finish
".build-venv-windows\Scripts\python.exe" -X utf8 scripts\build_desktop.py
set "ESTIMATOR_EXIT=%ERRORLEVEL%"
